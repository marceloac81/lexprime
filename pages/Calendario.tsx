        ...
        <div className={`animate-fade-in pb-20 relative min-h-full ${theme === 'hybrid' ? 'bg-[#222e35]' : ''}`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                            <h1 className={`text-2xl md:text-3xl font-bold tracking-tight ${theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-900 dark:text-white'}`}>Calendário</h1>
                            <CalendarIcon size={24} className={theme === 'hybrid' ? 'text-[#00a884]' : 'text-primary-500'} />
                        </div>
                        <p className={`text-sm mt-1 ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-500 dark:text-slate-400'}`}>Gerencie seus compromissos e prazos.</p>
                    </div>
                    <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                        <div className={`flex rounded-lg border shadow-sm relative date-picker-container z-20 ${theme === 'hybrid' ? 'bg-[#202c33] border-[#202c33]' : 'bg-white dark:bg-dark-800 border-slate-200 dark:border-slate-700'}`}>
                            <button onClick={goToToday} className={`px-3 py-2 text-xs font-bold border-r transition-colors uppercase tracking-wider ${theme === 'hybrid' ? 'border-[#111b21] text-[#aebac1] hover:bg-[#2a3942]' : 'text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                                HOJE
                            </button>
                        ...
                        <button
                            ...
                            className={`group p-2 rounded-lg transition-all flex items-center gap-2.5 text-left ${theme === 'hybrid' ? 'text-[#aebac1] hover:text-[#00a884] hover:bg-[#202c33]' : 'text-slate-500 hover:text-primary-600 dark:text-slate-400 dark:hover:text-primary-400 hover:bg-slate-100 dark:hover:bg-dark-700'}`}
            ...
            <div className="p-4 md:px-8 pt-2">
                <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0 pb-2">
                    {/* CALENDAR GRID */}
                    <div className={`flex-[3] rounded-2xl shadow-sm border overflow-hidden flex flex-col min-h-[600px] relative ${theme === 'hybrid' ? 'bg-[#2a3942] border-[#354751]' : 'bg-white dark:bg-dark-800 border-slate-200 dark:border-slate-700'}`}>
                        <div className={`grid grid-cols-7 border-b shrink-0 ${theme === 'hybrid' ? 'border-[#354751]' : 'border-slate-100 dark:border-slate-700'}`}>
                            {['dom', 'seg', 'ter', 'qua', 'qui', 'sex', 'sáb'].map(d => (
                                <div key={d} className={`py-3 text-center text-[10px] font-bold uppercase tracking-widest ${theme === 'hybrid' ? 'text-[#aebac1] bg-[#202c33]/50' : 'text-slate-400 bg-slate-50/50 dark:bg-dark-900/50'}`}>{d}</div>
                            ))}
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar" style={{ direction: 'rtl' }}>
                            <div className="grid grid-cols-7 auto-rows-fr" style={{ direction: 'ltr' }}>
                                {days.map((day, idx) => {
                                    if (day === null) return <div key={idx} className={`border-b border-r ${theme === 'hybrid' ? 'bg-[#202c33]/30 border-[#354751]' : 'bg-slate-50/30 dark:bg-dark-900/30 border-slate-100 dark:border-slate-800'}`} />;
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`border-b border-r p-1 md:p-2 min-h-[150px] md:min-h-[180px] relative group cursor-pointer transition-colors
                                ${theme === 'hybrid' 
                                    ? `border-[#354751] ${isSelected ? 'bg-[#00a884]/10 ring-2 ring-inset ring-[#00a884]/50' : 'hover:bg-[#202c33]/50'} ${isToday && !isSelected ? 'bg-[#00a884]/5' : ''}`
                                    : `${isSelected ? 'bg-primary-50 dark:bg-primary-900/10 ring-2 ring-inset ring-primary-500/50' : 'hover:bg-slate-50 dark:hover:bg-dark-900/50'} ${isToday && !isSelected ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''} border-slate-100 dark:border-slate-800`
                                }
                            `}
                                        >
                                            <span className={`text-sm font-bold block mb-2 w-7 h-7 flex items-center justify-center rounded-full 
                                ${isToday 
                                    ? (theme === 'hybrid' ? 'bg-[#00a884] text-white shadow-md' : 'bg-primary-600 text-white shadow-md') 
                                    : (theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-700 dark:text-slate-300')
                                }
                                ${isSelected && !isToday ? (theme === 'hybrid' ? 'bg-[#354751] text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white') : ''}
                            `}>
                                                {day}
                                            </span>
                    ...
                    {/* EVENTS PANEL */}
                    <div className={`flex-1 min-w-[320px] lg:max-w-md flex flex-col rounded-2xl border shadow-sm overflow-hidden ${theme === 'hybrid' ? 'bg-[#2a3942] border-[#354751]' : 'bg-white dark:bg-dark-800 border-slate-200 dark:border-slate-700'}`}>
                        <div className={`p-6 border-b flex justify-between items-center ${theme === 'hybrid' ? 'border-[#354751] bg-[#202c33]/50' : 'border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-dark-900/50'}`}>
                            <div>
                                <h2 className={`text-xl font-bold capitalize ${theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-900 dark:text-white'}`}>
                                ...
                                <p className={`text-xs mt-1 ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-500'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                            {selectedDeadlines.length === 0 && selectedAppointments.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-slate-400 py-12">
                                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${theme === 'hybrid' ? 'bg-[#354751] text-[#aebac1]' : 'bg-slate-50 dark:bg-dark-900 text-slate-300'}`}>
                                        <Clock size={32} />
                                    </div>
                                    <p className={`text-sm font-medium ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-400'}`}>Nenhum compromisso para hoje</p>
                                </div>
                            ) : (
                                <>
                                    {selectedDeadlines
                                        ...
                                        .map(deadline => {
                                            ...
                                            return (
                                                <div key={deadline.id} className={`group p-4 rounded-xl border shadow-sm hover:shadow-md transition-all relative overflow-hidden ${theme === 'hybrid' ? 'bg-[#354751] border-[#00a884]/30 hover:border-[#00a884]/50' : 'bg-white dark:bg-dark-900 border-slate-100 dark:border-slate-700'}`}>
                                                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${deadline.priority === 'High' ? 'bg-rose-500' : 'bg-amber-500'}`} />
                                                    <div className="flex justify-between items-start">
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <span className={`text-xs font-mono font-bold transition-colors ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-400'}`}>{(deadline.startTime || 'O dia todo').slice(0, 5)}</span>
                                                                <span className={`text-[9px] uppercase font-black px-2 py-0.5 rounded-full ${statusColor}`}>
                                                                    {statusLabel}
                                                                </span>
                                                            </div>
                                                            <h4 className={`font-bold leading-tight mb-0.5 truncate ${theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-800 dark:text-slate-200'}`}>{deadline.title}</h4>
                                                            <p className={`text-sm font-semibold truncate mb-1 ${theme === 'hybrid' ? 'text-[#d1d7db]' : 'text-slate-600 dark:text-slate-400'}`}>{clientName}</p>
                                                            <div className={`text-xs font-mono ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-400 dark:text-slate-500'}`}>
                                                                {procNum}
                                                            </div>
                                                        </div>
                                                        <div className="flex flex-col items-end shrink-0 ml-2">
                                                            <button onClick={() => handleEditDeadline(deadline)} className={`p-1 transition-colors ${theme === 'hybrid' ? 'text-[#354751] hover:text-[#00a884]' : 'text-slate-300 hover:text-primary-500'}`}>
                                                                <Edit size={16} />
                                                            </button>
                                                        </div>
                                                    </div>
                                    ...
                                    {selectedAppointments.map(appointment => (
                                        <div key={appointment.id} className={`p-4 rounded-xl border shadow-sm hover:shadow-md transition-all relative overflow-hidden ${theme === 'hybrid' ? 'bg-[#354751] border-[#00a884]/30 hover:border-[#00a884]/50' : 'bg-white dark:bg-dark-900 border-slate-100 dark:border-slate-700'}`}>
                                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${theme === 'hybrid' ? 'bg-[#00a884]' : 'bg-primary-500'}`} />
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className={`text-xs font-mono font-bold ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-400'}`}>{appointment.date.substring(11, 16) || 'O dia todo'}</span>
                                                        <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${theme === 'hybrid' ? 'bg-[#00a884]/10 text-[#00a884]' : 'bg-primary-50 text-primary-600 dark:bg-primary-900/20 dark:text-primary-400'}`}>Evento</span>
                                                    </div>
                                                    <h4 className={`font-bold leading-tight mb-1 ${theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-800 dark:text-slate-200'}`}>{appointment.title}</h4>
                                                    {appointment.location && (
                                                        <p className={`text-xs flex items-center gap-1 mt-1 ${theme === 'hybrid' ? 'text-[#d1d7db]' : 'text-slate-500'}`}>
                            ...
                            <button
                                onClick={() => setShowNewModal(true)}
                                className={`w-full py-3 mt-4 border rounded-xl font-bold text-xs uppercase tracking-widest transition-all border-dashed ${theme === 'hybrid' ? 'bg-[#354751] border-[#425866] text-[#e9edef] hover:bg-[#425866]' : 'bg-slate-50 dark:bg-dark-900 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-dark-700'}`}
                            >
                                Adicionar Item
                            </button>
                        </div>
                ...
                {showSyncModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
                        <div className={`rounded-2xl shadow-2xl border w-full max-w-lg overflow-hidden animate-scale-in ${theme === 'hybrid' ? 'bg-[#2a3942] border-[#354751]' : 'bg-white dark:bg-dark-800 border-slate-200 dark:border-slate-700'}`}>
                            <div className={`p-6 border-b flex justify-between items-center ${theme === 'hybrid' ? 'border-[#354751] bg-[#202c33]/50' : 'border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-dark-900/50'}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${theme === 'hybrid' ? 'bg-[#00a884]/10 text-[#00a884]' : 'bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400'}`}>
                                        <RefreshCw size={24} />
                                    </div>
                                    <div>
                                        <h3 className={`text-xl font-bold ${theme === 'hybrid' ? 'text-[#e9edef]' : 'text-slate-900 dark:white'}`}>Sincronizar Calendário</h3>
                                        <p className={`text-xs ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-500'}`}>Outlook, Google Calendar e iPhone</p>
                                    </div>
                                </div>
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between items-end mb-1">
                                        <label className={`text-xs font-bold uppercase tracking-wider ${theme === 'hybrid' ? 'text-[#aebac1]/60' : 'text-slate-400'}`}>Seu Link de Feed (iCal)</label>
                                        <button
                                    ...
                                    <div className={`flex gap-2 p-1.5 border rounded-xl items-center ${theme === 'hybrid' ? 'bg-[#202c33] border-[#354751]' : 'bg-slate-50 dark:bg-dark-900 border-slate-200 dark:border-slate-700'}`}>
                                        <div className={`flex-1 px-3 py-2 text-sm font-mono truncate ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-600 dark:text-slate-300'}`}>
                                            {syncToken ? (
                                                `${(import.meta as any).env.VITE_SUPABASE_URL}/functions/v1/calendar-feed?token=${syncToken}`
                                            ) : (
                                                <div className={`flex items-center gap-2 italic ${theme === 'hybrid' ? 'text-[#aebac1]/40' : 'text-slate-400'}`}>
                                        ...
                                        <button
                                            ...
                                            disabled={!syncToken}
                                            className={`${copied ? 'bg-green-500 border-green-500 text-white' : (theme === 'hybrid' ? 'bg-[#354751] border-[#425866] text-[#e9edef] hover:bg-[#425866]' : 'bg-white dark:bg-dark-800 text-slate-600 dark:text-slate-200 border-slate-200 dark:border-slate-600 hover:opacity-90')} px-4 py-2 rounded-lg border text-sm font-bold flex items-center gap-2 transition-all shadow-sm active:scale-95 min-w-[110px] justify-center disabled:opacity-50`}
                                        >
                                ...
                                <div className="space-y-4">
                                    <div className={`flex border-b ${theme === 'hybrid' ? 'border-[#354751]' : 'border-slate-100 dark:border-slate-700'}`}>
                                        <button
                                            onClick={() => setActiveSyncTab('outlook')}
                                            className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center justify-center gap-2 ${activeSyncTab === 'outlook' ? (theme === 'hybrid' ? 'border-[#00a884] text-[#00a884] font-black' : 'border-primary-500 text-primary-600 font-black') : (theme === 'hybrid' ? 'border-transparent text-[#aebac1]/60 hover:text-[#aebac1]' : 'border-transparent text-slate-400 hover:text-slate-600')}`}
                                        >
                                            <CalendarIcon size={14} /> Outlook
                                        </button>
                                        <button
                                            onClick={() => setActiveSyncTab('google')}
                                            className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center justify-center gap-2 ${activeSyncTab === 'google' ? (theme === 'hybrid' ? 'border-[#00a884] text-[#00a884] font-black' : 'border-primary-500 text-primary-600 font-black') : (theme === 'hybrid' ? 'border-transparent text-[#aebac1]/60 hover:text-[#aebac1]' : 'border-transparent text-slate-400 hover:text-slate-600')}`}
                                        >
                                            <Globe size={14} /> Google
                                        </button>
                                        <button
                                            onClick={() => setActiveSyncTab('iphone')}
                                            className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center justify-center gap-2 ${activeSyncTab === 'iphone' ? (theme === 'hybrid' ? 'border-[#00a884] text-[#00a884] font-black' : 'border-primary-500 text-primary-600 font-black') : (theme === 'hybrid' ? 'border-transparent text-[#aebac1]/60 hover:text-[#aebac1]' : 'border-transparent text-slate-400 hover:text-slate-600')}`}
                                        >
                                    ...
                                    <div className="min-h-[140px]">
                                        {activeSyncTab === 'outlook' && (
                                            <ol className="space-y-3 animate-fade-in">
                                                <li className={`flex gap-3 text-sm ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-600 dark:text-slate-400'}`}>
                                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${theme === 'hybrid' ? 'bg-[#202c33] text-[#00a884]' : 'bg-slate-100 dark:bg-dark-700'}`}>1</span>
                                                    <span>No Outlook, clique em <strong>Adicionar Calendário</strong> &gt; <strong>Da Internet</strong>.</span>
                                                </li>
                                                <li className={`flex gap-3 text-sm ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-600 dark:text-slate-400'}`}>
                                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${theme === 'hybrid' ? 'bg-[#202c33] text-[#00a884]' : 'bg-slate-100 dark:bg-dark-700'}`}>2</span>
                                        ...
                                        {activeSyncTab === 'iphone' && (
                                            <ol className="space-y-3 animate-fade-in">
                                                ...
                                                <li className={`flex gap-3 text-sm ${theme === 'hybrid' ? 'text-[#aebac1]' : 'text-slate-600 dark:text-slate-400'}`}>
                                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${theme === 'hybrid' ? 'bg-[#202c33] text-[#00a884]' : 'bg-slate-100 dark:bg-dark-700'}`}>3</span>
                            ...
                            <div className={`p-6 border-t flex justify-end ${theme === 'hybrid' ? 'bg-[#202c33]/30 border-[#354751]' : 'bg-slate-50 dark:bg-dark-900/50 border-slate-100 dark:border-slate-700'}`}>
                                <button
                                    onClick={() => setShowSyncModal(false)}
                                    className={`px-6 py-2.5 rounded-xl font-bold text-sm hover:opacity-90 transition-opacity ${theme === 'hybrid' ? 'bg-[#00a884] text-white' : 'bg-slate-900 dark:bg-white text-white dark:text-slate-900'}`}
                                >
                                    Concluído
                                </button>
                            </div>
